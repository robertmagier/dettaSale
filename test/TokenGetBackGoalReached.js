...
contract("Deta Sale", function (accounts) {
  ...
  it("Finalize sale", async function () {
    ...
    changeTime(110);
    let weiBalance = await ethProvider.getBalance(user.public);
    await expect(saleInstance.finalize()).to.be.eventually.fulfilled;
    let weiBalanceAfter = await ethProvider.getBalance(user.public);
    expect(weiBalanceAfter.toString()).to.be.equal(
      weiBalance.add(2).toString()
    );
    ...
    await expect(
      saleInstance.claimRefund(buyerAccount)
