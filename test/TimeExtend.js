...
contract("Deta Sale", function (accounts) {
  ...
  it("Check Time Extend", async function () {
    changeTime(20);
    await expect(saleInstance.extendTime(120)).to.be.eventually.fulfilled;
    await expect(saleInstance.extendTime(110)).to.be.eventually.rejected;
    changeTime(130);
    await expect(saleInstance.extendTime(140)).to.be.eventually.rejected;
  });
});
